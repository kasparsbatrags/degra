...
public class FreightTrackingSecurityConfig {
	...
	public SecurityFilterChain freightTrackingSecurityFilterChain(HttpSecurity http) throws Exception {
		http.securityMatcher(FREIGHT_TRACKING_PATH + "/**").cors(cors -> cors.configurationSource(corsConfigurationSource())).csrf(csrf -> {
				...
				.authorizeHttpRequests(
						authz -> authz.requestMatchers(HttpMethod.OPTIONS, "/**").permitAll().requestMatchers(FREIGHT_TRACKING_PATH + "/**")
	...
	static class ImprovedRoleConverter implements Converter<Jwt, Collection<GrantedAuthority>> {
		...
		public Collection<GrantedAuthority> convert(Jwt jwt) {
			...
			return roles.stream().distinct() // Noņem dublikātus
