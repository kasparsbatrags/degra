...
export const normalizeRoutePageFromApi = (apiData: RawApiRoutePage): RoutePage | null => {
  try {
    ...
    const truckRegistrationNumber = apiData.truck?.registrationNumber || 
                                   (apiData as any).truck_registration_number ||
                                   (apiData as any).truckRegistrationNumber;
    ...
    const fuelConsumptionNorm = apiData.fuelConsumptionNorm || 
                               (apiData as any).fuel_consumption_norm ||
                               0;
    ...
    const fuelBalanceAtStart = apiData.fuelBalanceAtStart || 
                              (apiData as any).fuel_balance_at_start ||
                              0;
    ...
    const normalized: RoutePage = {
      ...
      total_fuel_received_on_routes: apiData.totalFuelReceivedOnRoutes ?? 
                                    (apiData as any).total_fuel_received_on_routes ?? 
                                    undefined,
      total_fuel_consumed_on_routes: apiData.totalFuelConsumedOnRoutes ?? 
                                    (apiData as any).total_fuel_consumed_on_routes ?? 
                                    undefined,
      fuel_balance_at_routes_finish: apiData.fuelBalanceAtRoutesFinish ?? 
                                    (apiData as any).fuel_balance_at_routes_finish ?? 
                                    undefined,
      odometer_at_route_start: apiData.odometerAtRouteStart ?? 
                              (apiData as any).odometer_at_route_start ?? 
                              undefined,
      odometer_at_route_finish: apiData.odometerAtRouteFinish ?? 
                               (apiData as any).odometer_at_route_finish ?? 
                               undefined,
      computed_total_routes_length: apiData.computedTotalRoutesLength ?? 
                                   (apiData as any).computed_total_routes_length ?? 
                                   undefined,
      is_dirty: 0,
      is_deleted: 0,
      created_at: Date.now(),
    ...
    console.log('🔄 [NORMALIZER] Successfully normalized route page:', {
      id: normalized.id,
      truck: normalized.truck_registration_number,
