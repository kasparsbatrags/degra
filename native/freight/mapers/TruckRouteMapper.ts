import {CreateTruckRouteDto, TruckRouteDto} from '@/dto/TruckRouteDto'
...
export class TruckRouteMapper {
	...
	static sqlToTruckRouteDto(sqlResult: TruckRouteSqlResult): TruckRouteDto {
		...
		const truckRoutePage: TruckRoutePageDto = {
			...
			truck: truck,
			user: user,
			fuelBalanceAtStart: sqlResult.truckRoutePageFuelBalanceAtStart,
	...
	return routes.map((route, index) => {
		...
		if (!isValid) {
			console.warn('📱 [WARN] Filtering out invalid route:', {
				truckRoutePageUid: route.truckRoutePageUid, routeDate: route.routeDate, outTruckObjectUid: route.outTruckObjectUid
			})
		}
		return isValid
	})
