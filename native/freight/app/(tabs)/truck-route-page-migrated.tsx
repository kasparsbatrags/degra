...
export default function TruckRoutePageScreen() {
	...
	const {
		...
		id ? `${CACHE_KEYS.ROUTE_PAGE}_${id}` : null,
		async () => {
			if (!id) return null;
	...
	const {
		...
		id ? `${CACHE_KEYS.TRUCK_ROUTES}_${id}_${pagination.page}` : null,
		async () => {
	...
	const {
		...
		submitForm,
		hasUnsavedChanges
	} = useOfflineForm<TruckRoutePageForm>(
		{
			dateFrom: new Date(),
			dateTo: new Date(),
			truck: '',
		...
		async (formData) => {
			const payload = {
				...
				truck: {id: parseInt(formData.truck)},
				fuelBalanceAtStart: parseFloat(formData.fuelBalanceAtStart),
				fuelBalanceAtFinish: formData.fuelBalanceAtFinish ? parseFloat(formData.fuelBalanceAtFinish) : null,
			}
	...
	const handleSubmit = async () => {
		if (!isOnline) {
			...
			const confirmed = confirm(
				"Offline režīms: Dati tiks saglabāti lokāli un nosūtīti, kad būs pieejams internets. Turpināt?"
			)
			if (!confirmed) return
		}
		...
		<SafeAreaView style={styles.container}>
			<ScrollView>
				<View style={styles.content}>
					...
					{/* NEW: Show unsaved changes indicator */}
					{hasUnsavedChanges && (
