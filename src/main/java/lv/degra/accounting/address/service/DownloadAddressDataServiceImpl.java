...
import lv.degra.accounting.address.model.CsvDataBase;
...
import static lv.degra.accounting.address.enums.ArRecordStatus.statusOnSystemByCode;
...
public class DownloadAddressDataServiceImpl implements DownloadAddressDataService {
    ...
    private void importFile(InputStreamReader file, ArZipContentFiles fileInfo) {
        ...
        List<Address> addressList = csvData.parallelStream()
                .map(row -> objectMapper.convertValue(row, Address.class))
